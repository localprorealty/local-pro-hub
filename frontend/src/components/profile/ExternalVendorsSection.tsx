...
export function ExternalVendorsSection() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null)
    try {
      const [vendorList, status] = await Promise.all([
        fetchAgentVendors(),
        fetchGmailCredentialsStatus(),
      ])
      ...
      console.error('Failed to load vendors or credentials:', err)
      setError(err instanceof Error ? err.message : 'Failed to load vendor settings')
    } finally {
      setIsLoading(false)
    }
  }, [])
    ...
    <div className="border-t border-[var(--color-border)] pt-8">
      ...
      <div className="mb-6 space-y-1">
        ...
        <h3 className="font-[family-name:var(--font-display)] text-lg font-semibold text-[var(--color-white)]">
          Preferred Photographer & Email Dispatch
        </h3>
        <p className="text-xs text-[var(--color-text-secondary)]">
          Configure your external photographer vendor and optional direct Gmail sending.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {/* Card 1: Preferred Photographer Vendor */}
        <div className="rounded-xl border border-[var(--color-border)] bg-[#101010] p-5 space-y-4">
          <div className="flex items-center justify-between border-b border-[var(--color-border)]/60 pb-3">
            <div className="flex items-center gap-2.5">
              ...
              <h4 className="text-sm font-semibold text-white">External Photographer</h4>
            </div>
            {!isEditingVendor && (
              <Button
                type="button"
        ...
        {/* Card 2: Gmail App Password Integration */}
        <div className="rounded-xl border border-[var(--color-border)] bg-[#101010] p-5 space-y-4">
          <div className="flex items-center justify-between border-b border-[var(--color-border)]/60 pb-3">
            <div className="flex items-center gap-2">
            ...
            <div className="rounded-lg border border-[var(--color-gold)]/30 bg-[#17150e] p-3.5 text-xs text-zinc-300 space-y-2.5 relative">
              ...
              <div className="leading-relaxed space-y-1">
                ...
                <ol className="list-decimal pl-4 space-y-0.5 text-zinc-300">
                  ...
                  <li>
                    Name it <em>&ldquo;LocalPRO Hub&rdquo;</em> and copy the generated 16-letter
                    code.
                  </li>
                </ol>
              </div>
